...
class SPP(nn.Module):
    ...
    def __init__(self, ch=128, kernel_sizes=[5, 9, 13], stride=1):
        ...
        self.conv1 = nn.Conv2d(ch, hidden_ch, 1, 1, bias=False)
        self.conv2 = nn.Conv2d(hidden_ch * (len(kernel_sizes) + 1), ch, 1, 1, bias=False)
...
class Convolution(nn.Module):
    def __init__(self,
        ...
        self.convolution = nn.Conv2d(in_ch,
                                     out_ch,
                                     kernel_size,
                                     stride,
...
class StackedHourglass(nn.Module):
    def __init__(self, num_stack: int, in_ch: int, out_ch: int, increase_ch: int = 0, activation: str = 'ReLU', pool: str = 'Max', neck_activation: str = 'ReLU', neck_pool: str = 'None', num_seq: int = 1):
        super(StackedHourglass, self).__init__()

        # downsample the resolution of input (1 --> 1/4(scale_factor))
        self.pre_layer = PreLayer(in_ch=3 * num_seq, mid_ch=128, out_ch=in_ch, activation=activation, pool=pool)
...
def get_stacked_hourglass(args):
    model = StackedHourglass(num_stack=args.num_stack,
